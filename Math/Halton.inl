...
namespace Math {
    ...
    static std::array<float, Dimensionality> Halton::Element(uint32_t elementIndex, std::optional<std::array<uint32_t, Dimensionality>> customBases) {
        uint32_t d = 0;
        uint32_t j = 0;
    ...
    template <uint32_t Dimensionality>
    static std::vector<std::array<float, Dimensionality>> Halton::Sequence(uint32_t elementStartIndex, uint32_t elementEndIndex) {
