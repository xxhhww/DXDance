...
namespace Core {
	...
	Actor* World::FindActorByID(int32_t id) {
		auto it = std::find_if(mActors.begin(), mActors.end(),
			[&](std::unique_ptr<Actor>& item) {
	...
	Actor* World::FindActorByName(const std::string& name) {
		auto it = std::find_if(mActors.begin(), mActors.end(),
			[&](std::unique_ptr<Actor>& item) {
	...
	void World::SerializeJson(Tool::JsonWriter& writer) const {
		writer.StartObject();
	...
	void World::DeserializeJson(const Tool::JsonReader& reader) {
