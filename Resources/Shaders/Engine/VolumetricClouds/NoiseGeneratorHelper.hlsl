#ifndef _NoiseGeneratorHelper__
#define _NoiseGeneratorHelper__

//---------------------------------------------------------------------------------------------------------
// Math Functions
//---------------------------------------------------------------------------------------------------------
float3 interpolationC2(float3 x) { 
...
float3 setRangesSigned(float3 values, float low, float high) { 
...
float dilatePerlinWorley(float p, float w, float x) {
	...
	if(x < 0.5f){
		x /= 0.5f;
		float n = p + w * x;
		return n * lerp(1.0f, 0.5f, pow(x,curve));
	}
	else{
		x = (x - 0.5f) / 0.5f;
		float n = w + p * (1.0f - x);
		return n * lerp(0.5f, 1.0f, pow(x, 1.0f / curve));
	}
}
