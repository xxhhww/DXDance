...
void GenerateNormalBubblesMap(uint3 id : SV_DispatchThreadID) {
    ...
    // 以当前点为中心，获取周围4个点的偏移值
    float3 x1D = displaceMap[uvX1].xyz; // 在x轴 第一个点的偏移值
    float3 x2D = displaceMap[uvX2].xyz; // 在x轴 第二个点的偏移值
    float3 z1D = displaceMap[uvZ1].xyz; // 在z轴 第一个点的偏移值
    float3 z2D = displaceMap[uvZ2].xyz; // 在z轴 第二个点的偏移值
