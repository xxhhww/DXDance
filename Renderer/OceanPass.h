...
namespace Renderer {
	...
	public:
		struct OceanBuilderData {
			Math::Vector4 WindAndSeed = Math::Vector4(1.0f, 1.0f, 0.0f, 0.0f);	// 风和随机种子 xy为风, zw为两个随机种子

			float WindScale = 30.0f;
			int N;					// FFT纹理大小
			float OceanLength;		// 海洋长度
			float A = 73.0f;		// phillips谱参数，影响波浪高度

			int Ns;					// Ns = pow(2,m-1); m为第几阶段
			float Lambda = 8.0f;		// 偏移影响
			float HeightScale = 28.8f;	// 高度影响
			float BubblesScale = 1;		// 泡沫强度

			float BubblesThreshold = 0.86; // 泡沫阈值
	...
	public:
		...
		inline static int smFFTPow = 9u;				// 生成海洋纹理大小2的次幂
		inline static int smFFTSize = pow(2, smFFTPow);	// fft纹理大小 = pow(2, fftPow)
		inline static int smMeshSize = 100;			//网格长宽数量
		inline static float smMeshLength = 512.0f;	//网格长度
