...
namespace Renderer {
	...
	public:
		...
		inline RvtPageTableNodeCell& GetCell(int x, int y, int mipLevel) { return mPageLevelTables.at(mipLevel).GetCell(x, y); }
		inline const RvtPageTableNodeCell& GetCell(int x, int y, int mipLevel) const { return mPageLevelTables.at(mipLevel).GetCell(x, y); }
