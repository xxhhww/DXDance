...
namespace Renderer {
	...
	void GrassPass::InitializePass(RenderEngine* renderEngine) {
		...
		// 加载grassMesh
		{
			...
			grassMesh = std::make_unique<Renderer::Mesh>(
				device,
			...
			grassMesh->LoadDataFromMemory(copyDsQueue, copyFence, vertices, indices);
			grassMesh->GetVertexBuffer()->SetDebugName("FoliageMeshVertexBuffer");
			grassMesh->GetIndexBuffer()->SetDebugName("FoliageMeshIndexBuffer");
		}
		*/
