...
namespace OfflineTask {
	...
	void SplitHeightMap::Run(const std::string& filename, float tileCountPerAxis) {
        ...
        dstImage.width = srcImage.GetImages()[0].width / tileCountPerAxis;
        dstImage.height = srcImage.GetImages()[0].height / tileCountPerAxis;
        dstImage.format = srcImage.GetImages()[0].format;
        dstImage.rowPitch = srcImage.GetImages()[0].rowPitch / tileCountPerAxis;
        dstImage.slicePitch = srcImage.GetImages()[0].slicePitch / (tileCountPerAxis * tileCountPerAxis);
        dstImage.pixels = new uint8_t[dstImage.slicePitch];

        for (uint32_t row = 0; row < tileCountPerAxis; row++) {
            for (uint32_t col = 0; col < tileCountPerAxis; col++) {

                // 计算均分后图像相对于原始图形的起始偏移量
                uint64_t srcOffset = (tileCountPerAxis * row) * dstImage.slicePitch + col * dstImage.rowPitch;
