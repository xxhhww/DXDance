...
namespace Windows
{
	...
	*/
	...
	public:
		...
		* @param p_dialogTitle
		*/
		FileDialog(std::function<int(tagOFNW*)> p_callback, const std::string& p_dialogTitle);
	...
	protected:
		std::function<int(tagOFNW*)> m_callback;
		...
		std::wstring m_filterW;
		std::wstring m_filepathW;
		std::wstring m_dialogTitlerW;
		std::wstring m_initialDirectoryW;
	};
}