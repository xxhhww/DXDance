...
namespace Windows {
	...
	FileDialog::FileDialog(std::function<int(tagOFNW*)> p_callback, const std::string& p_dialogTitle) :
	...
	{
		m_filterW = Tool::StrUtil::UTF8ToWString(m_filter);
		m_filepathW = Tool::StrUtil::UTF8ToWString(m_filepath);
		m_dialogTitlerW = Tool::StrUtil::UTF8ToWString(m_dialogTitle);
		m_initialDirectoryW = Tool::StrUtil::UTF8ToWString(m_initialDirectory);
		...
		m_filepathW.resize(MAX_PATH);

		ZeroMemory(&ofn, sizeof(ofn));
		ofn.lStructSize = sizeof(ofn);
		ofn.hwndOwner = NULL;  // If you have a window to center over, put its HANDLE here
		ofn.lpstrFilter = m_filterW.c_str();
		ofn.lpstrFile = m_filepathW.data();
		ofn.nMaxFile = MAX_PATH;
		ofn.lpstrTitle = m_dialogTitlerW.c_str();

		if (!m_initialDirectory.empty())
			ofn.lpstrInitialDir = m_initialDirectoryW.c_str();
